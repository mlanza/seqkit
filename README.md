...
Your local-first commonplace book is a near perfect spot for keeping all the information and instructions an agent needs to thrive.  It's the ideal store for skills: one place to rule them all.  Tag a page `Skills` and describe it with a `description` property.  Include any `prerequisites` that make sense and you're ready to go.  Prerequisite topics are automatically — and recursively — included when calling `about`.
